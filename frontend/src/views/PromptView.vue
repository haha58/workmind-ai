...
<template>
  <div class="prompt-view">
    <div class="top-tabs">
      <button v-for="tab in tabs" :key="tab.id" class="top-tab" :class="{ active: activeTab === tab.id }"
          ...
          <div class="panel-section">
            <div class="section-row">
              ...
              <div class="section-actions">
                <button class="btn-text" @click="showTemplates = !showTemplates">
                  <el-icon>
                    <FolderOpened />
                  </el-icon> 从模板加载
                </button>
            ...
            <textarea v-model="ps.testConfig.systemPrompt" class="input editor-area"
              placeholder="输入 System Prompt（可为空）..." rows="6" />
            <div class="char-info">
          ...
          <div class="params-row">
            <div class="param-item">
              ...
              <div class="slider-wrap">
                <input type="range" min="0" max="2" step="0.1" v-model.number="ps.testConfig.temperature"
                  class="slider" />
            ...
            <div class="param-item">
              ...
              <input type="number" min="100" max="4096" step="100" v-model.number="ps.testConfig.maxTokens"
                class="input param-input" />
            </div>
          </div>
          <button class="btn btn-primary run-btn" @click="ps.runTest()"
            :disabled="!ps.testConfig.userMessage.trim() || ps.testing">
            <template v-if="ps.testing"><el-icon>
                <Loading />
              </el-icon> 测试中...</template>
            <template v-else><el-icon>
                <CaretRight />
              </el-icon> 运行测试</template>
          </button>
        </div>
        <div class="result-panel">
          <div v-if="ps.testResult.totalTokens" class="metrics-bar">
            <span class="metric"><el-icon>
                <Timer />
              </el-icon>{{ ps.testResult.latencyMs }}ms</span>
            <span class="metric"><el-icon>
                <Download />
              </el-icon>{{ ps.testResult.inputTokens }}</span>
            <span class="metric"><el-icon>
                <Upload />
              </el-icon>{{ ps.testResult.outputTokens }}</span>
            <span class="metric"><el-icon>
                <Coin />
              </el-icon>¥{{
          ...
          <div class="result-content">
            <div v-if="!ps.testResult.content && !ps.testing" class="result-empty">
              运行测试后，AI 回复将在此显示
            </div>
      ...
      <div v-if="showTemplates" class="overlay" @click.self="showTemplates = false">
        <div class="template-picker">
          <div class="picker-header">
            <span>选择模板</span><button class="btn-close" @click="showTemplates = false">×</button>
          </div>
      ...
      <div class="ab-columns">
        <div class="ab-col" :class="{ winner: ps.abResult.evaluation?.winner === 'A' }">
          <div class="ab-col-header">
            <span class="ab-label ab-a">A</span><span class="ab-col-title">System Prompt A</span>
          </div>
          <textarea v-model="ps.abConfig.systemPromptA" class="input ab-input" placeholder="System Prompt A..."
            rows="5" />
          ...
          <div v-if="ps.abResult.evaluation?.winner === 'A'" class="winner-badge">
            <el-icon>
              <Trophy />
            </el-icon> 胜出
          </div>
        ...
        <div class="ab-col" :class="{ winner: ps.abResult.evaluation?.winner === 'B' }">
          <div class="ab-col-header">
            <span class="ab-label ab-b">B</span><span class="ab-col-title">System Prompt B</span>
          </div>
          <textarea v-model="ps.abConfig.systemPromptB" class="input ab-input" placeholder="System Prompt B..."
            rows="5" />
          ...
          <div v-if="ps.abResult.evaluation?.winner === 'B'" class="winner-badge">
            <el-icon>
              <Trophy />
            </el-icon> 胜出
          </div>
    ...
    <div v-if="activeTab === 'templates'" class="tab-panel">
      <div class="template-manager">
        <div class="tpl-list-panel">
          ...
          <div class="tpl-list">
            <div v-for="t in ps.templates" :key="t.id" class="tpl-list-item" :class="{ selected: selectedId === t.id }"
              @click="selectTpl(t)">
          ...
          <div v-else class="tpl-edit-form">
            <div class="tpl-edit-header">
              ...
              <div class="tpl-edit-actions">
                ...
                <button class="btn btn-ghost btn-sm" @click="doDelete" :disabled="editing.id?.startsWith('t_default_')">
                  删除
                </button>
            ...
            <label class="field-label">System Prompt</label>
            <textarea v-model="editing.systemPrompt" class="input tpl-prompt-area" rows="10"
            ...
            <div v-if="editing.versions?.length" class="version-history">
              ...
              <div v-for="v in [...editing.versions].reverse()" :key="v.version" class="version-item">
                ...
                <button class="version-restore" @click="
