        ...
        onEvent: (event, data) => {
          ...
          if (event === "tool_call") {
            const newStep = {
              ...
              durationMs: null, // 等待 tool_result 事件填充
              runId: data.runId, // 用于匹配
            };
            task.steps.push(newStep);
            console.log('[tool_call] 添加步骤:', newStep.id, 'runId:', data.runId);
          }

          // 工具执行完毕：更新对应的步骤
          if (event === "tool_result") {
            console.log('[tool_result] 收到结果, runId:', data.runId, 'toolName:', data.toolName);
            console.log('[tool_result] 当前步骤:', task.steps.map(s => ({ id: s.id, toolName: s.toolName, status: s.status, runId: s.runId })));
            
            // 严格通过 runId 匹配
            let step = null;
            if (data.runId) {
              step = task.steps.find(s => s.runId === data.runId);
            }
            
            // 如果 runId 匹配失败，回退到按 toolName 和状态匹配（找第一个 running 的）
            if (!step) {
              step = task.steps.find(s => s.toolName === data.toolName && s.status === "running");
            }
            
            if (step) {
              console.log('[tool_result] 匹配到步骤:', step.id);
              step.result = data.resultText;
              step.status = "done";
              step.durationMs = data.durationMs || 1; // 使用后端传来的时间
            } else {
              console.warn('[tool_result] 未匹配到步骤!');
            }
          }
